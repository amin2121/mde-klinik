<!-- Second navbar -->
    ...
    <div class="navbar-collapse collapse" id="navbar-second-toggle">
  		<ul class="nav navbar-nav navbar-nav-material">
					<li><a href="<?= base_url('apotek/apotek_home') ?>"><i class="icon-dots position-left"></i> Home</a></li>
					<?php
						$level = $this->session->userdata('level');
						$q_menu = $this->db->query("SELECT a.* FROM pengaturan_hak_akses a LEFT JOIN menu_2 b ON a.id_menu = b.id WHERE a.level = '$level' AND a.tipe_menu = 'Menu 2' AND b.id_menu_1 = '3'")->result_array();
						foreach ($q_menu as $p) {
							$id_menu = $p['id_menu'];
