		...
		$('#btn_proses_bayar').click(function(){
			 ...
			 $.ajax({
					 ...
					 success : function(row){
							 var id_apotek_penjualan = row['id_apotek_penjualan'];
								...
								let data = `
									<tr id="row-${row.id}" class="row-${row.id}">
										<td>
											...
											<input type="text" size="5" class="form-control qty-${row.id}" id="qty-${row.id}" name="qty[]" onkeyup="hitung_total_harga_beli(this, ${row.id}, '${row.harga_jual}', '${row.laba}');" value="1">
										</td>
