...
function get_pasien(){
	...
	$.ajax({
			url : '<?php echo base_url(); ?>farmasi/kasir/get_pasien',
			data : {search:search},
			type : "POST",
			dataType : "json",
...
function klik_pasien(id){
	$('#tutup_data_pasien').click();
		...
		$.ajax({
				url : '<?php echo base_url(); ?>farmasi/kasir/klik_pasien',
				data : {id:id},
				type : "POST",
				dataType : "json",
				...
				<div class="col-sm-7">
					...
					<div class="panel panel-flat border-top-success border-top-lg">
						...
						<div class="panel-body">
							...
							<form action="<?= base_url('farmasi/kasir/transaksi_umum') ?>" method="POST" id="form-checkout">
								<div class="form-group">
									<div class="input-group">
...
<script>
	...
	$(window).load((e) => {
		...
		$('#btn_proses_bayar').click(function(){
			...
			$.ajax({
					 url : '<?php echo base_url(); ?>farmasi/kasir/transaksi_umum',
					 ...
					 success : function(row){
						...
						if (row['status'] == true) {
							 var win = window.open('<?php echo base_url(); ?>farmasi/kasir/cetak_struk/'+id_penjualan, '_blank','location=yes,height=570,width=520,scrollbars=yes,status=yes');
							 window.location.href = '<?php echo base_url(); ?>farmasi/kasir/umum';
						}else {
						 	window.location.href = '<?php echo base_url(); ?>farmasi/kasir/umum';
						}
				...
				if(res.status == false) {
					...
					for(const item of res.data) {
						...
						if (item.stok < 0) {
							...
							var cursor = `style="cursor:pointer;" onclick="add_row('${item.id_barang}', '${item.nama_barang}', '${item.kode_barang}', '${item.harga_jual}', '${item.laba}')"`;
						}
