									...
									<div class="tab-content">
												...
												<div class="col-sm-6">
													<div class="form-group">
														<div class="input-group">
															<input type="text" class="form-control" autocomplete="off" name="rak" id="nama_barang" onkeyup="get_barang()" placeholder="Cari Berdasarkan Nama Barang/Kode Barang">
										...
										<div class="tab-pane" id="tambah-nama-barang">
											<fieldset class="content-group">
												<form action="<?= base_url('farmasi/nama_barang/tambah_barang') ?>" id="form-tambah-barang" method="POST">
													...
													<div class="form-group">
														...
														<select class="select-search" name="id_satuan">
													...
													<div class="form-group">
														<label class="control-label"><b>Stok Minimal</b></label>
														<input type="text" class="form-control" placeholder="Stok Minimal" name="stok_minimal">
													</div>													
													...
													<div class="form-group" style="margin-top: 1.5em !important;">
														<button class="btn btn-success" type="submit"><i class="icon-floppy-disk position-left"></i> Tambah</button>
													</div>
												</form>
											</fieldset>
...
function get_barang() {
	...
	$.ajax({
		url : '<?= base_url('farmasi/nama_barang/get_nama_barang_with_jenis_barang_ajax') ?>',
		data : {'search_barang' : `${nama_barang}`},
		method : 'GET',
		dataType : 'json',
		beforeSend : () => {
		...
		success : (res) => {
			...
			if(res.status) {
				...
				for(const item of res.data) {
					...
					let tanggal = new String(item.tanggal).split("-").reverse().join("-") == 'null' ? '-' : new String(item.tanggal).split("-").reverse().join("-");
								...
								<div id="modal_edit_barang_${item.id}" class="modal fade">
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<form action="<?= base_url('farmasi/nama_barang/ubah_barang') ?>" method="POST">
											<div class="modal-body">
												...
												<div class="form-group">
													<label class="control-label"><b>Stok Minimal</b></label>
													<input type="text" class="form-control" placeholder="Stok Minimal" name="stok_minimal" value="${item.stok_minimal == null ? 0 : item.stok_minimal}">
												</div>
								...
								<div id="modal_hapus_barang_${item.id}" class="modal fade">
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-footer">
												<button class="btn btn-link" data-dismiss="modal"><i class="icon-cross position-left"></i> Keluar</button>
												<a href="<?= base_url('farmasi/nama_barang/hapus_barang?id_barang=') ?>${item.id}" class="btn btn-danger"><i class="icon-bin position-left"></i> Hapus</a>
											</div>
										</div>
									</div>
								</div>
